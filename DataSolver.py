...
class MiniImagenetHard(MiniImagenet):
    ...
    def getitem(self, index):
        ...
        for i in range(self.num_shot_per_task):
            ...
            im = self.transform(Image.open(os.path.join(self.data_folder, folders[ind], names[ind][indexa[i] % 600])))
            if im.size(0) == 1:
                im = im.repeat(3, 1, 1)
        ...
        for i in range(self.num_query_per_task):
            ...
            im = self.transform(Image.open(os.path.join(self.data_folder, folders[ind], names[ind][indexb[i] % 600])))
            if im.size(0) == 1:
                im = im.repeat(3, 1, 1)
